...
if luci.sys.call("pidof koolproxy >/dev/null") == 0 then
	status = translate("<strong><font color=\"green\">广告过滤VIP 运行中</font></strong>")
else
	status = translate("<strong><font color=\"red\">广告过滤VIP 已停止</font></strong>")
end

o = Map("koolproxy", translate("广告过滤VIP"), translate("koolproxy是能识别adblock规则的代理软件，可以过滤普通网页广告、视频广告、HTTPS广告<br />Adblock Plus 的 Host 列表 + KoolProxy 黑名单模式运行更流畅上网体验，开启全局模式获取更好的过滤效果<br /><font color=\"red\">如果要为客户端过滤https广告，必须在客户端安装根证书，输入网址110.110.110.110安装证书<br /></font>"))
...
e.description = translate("加入的网址不走广告过滤端口。只针对白名单模式。只能输入WEB地址，如：google.com，每个地址一行。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已经加入的IP地址将使用代理，但只有GFW模式。请输入IP地址或IP地址段，每行只能输入一个IP地址。例如:112.123.134.145/24或112.123.134.145。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
...
e.description = translate("这些已经加入的IP地址将不会使用过滤器。请输入IP地址或IP地址段，每行只能输入一个ip地址。例如:112.123.134.145/24或112.123.134.145。")
e.rows = 28
e.wrap = "off"
e.rmempty = false
